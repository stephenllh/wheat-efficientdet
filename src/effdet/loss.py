...
def huber_loss(input, target, delta=1.0, weights=None, size_average=True):
    """"""
    err = input - target
    abs_err = err.abs()
...
def smooth_l1_loss(input, target, beta=1.0 / 9, weights=None, size_average=True):
    """
...
def _classification_loss(
    cls_outputs, cls_targets, num_positives, alpha=0.25, gamma=2.0
):
...
def _box_loss(box_outputs, box_targets, num_positives, delta=0.1):
    ...
    box_loss = huber_loss(
        box_targets, box_outputs, weights=mask, delta=delta, size_average=False
    )
    box_loss /= normalizer
    return box_loss
...
class DetectionLoss(nn.Module):
    ...
    def forward(
        self, cls_outputs, box_outputs, cls_targets, box_targets, num_positives
    ):
        ...
        for level in range(levels):
            ...
            cls_targets_at_level_oh = F.one_hot(
                cls_targets_at_level * cls_targets_non_neg, self.num_classes
            )
            cls_targets_at_level_oh = torch.where(
                cls_targets_non_neg.unsqueeze(-1),
                cls_targets_at_level_oh,
                torch.zeros_like(cls_targets_at_level_oh),
            )
