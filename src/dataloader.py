...
class Dataset:
    def __init__(
        self, data_path, df, image_ids, transforms=None, do_cutmix=True, test=False
    ):
            ...
            while True:
                ...
                if len(sample["bboxes"]) > 0:
                    ...
                    target["boxes"][:, [0, 1, 2, 3]] = target["boxes"][
                        :, [1, 0, 3, 2]
                    ]  # y,x,y,x for efficientdet_pytorch repo CocoDataset class (self.yxyx=True for PyTorch)
    ...
    def load_image_and_boxes(self, index):
        ...
        df_images = self.df[
            self.df["image_id"] == image_id
        ]  # get all the rows for the image_id
    ...
    def load_cutmix_image_and_boxes(self, index, imsize=1024):
        ...
        np.clip(result_boxes[:, 0:], a_min=0, a_max=2 * s, out=result_boxes[:, 0:])
...
def get_train_loader(
    data_path, df, image_ids, transforms, do_cutmix, batch_size, num_workers
):
    train_dataset = Dataset(
        data_path, df, image_ids, transforms=transforms, do_cutmix=do_cutmix, test=False
    )
